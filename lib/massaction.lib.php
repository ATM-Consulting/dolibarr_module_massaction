...
function getHtmlSelectElements($entity, $TExcludeId=array(), $element='propal')
{
	...
	if($element == 'propal') $sql = 'SELECT p.rowid, p.ref,  p.total_ht, s.nom, s.code_client,  p.multicurrency_code as currency_code FROM '.MAIN_DB_PREFIX.'propal p';
	elseif($element == 'commande') $sql = 'SELECT p.rowid, p.ref,  p.total_ht, s.nom, s.code_client, p.multicurrency_code as currency_code FROM '.MAIN_DB_PREFIX.'commande p';
	elseif($element == 'facture') $sql = 'SELECT p.rowid, p.ref,  p.total_ht, s.nom, s.code_client, p.multicurrency_code as currency_code FROM '.MAIN_DB_PREFIX.'facture p';
