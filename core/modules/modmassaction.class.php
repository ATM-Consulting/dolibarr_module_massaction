...
class modmassaction extends DolibarrModules
{
	...
	function __construct($db)
	{
		...
		// Example: $this->tabs = array('objecttype:+tabname1:Title1:massaction@massaction:$user->hasRight('massaction', 'read'):/massaction/mynewtab1.php?id=__ID__',  	// To add a new tab identified by code tabname1
        //                              'objecttype:+tabname2:Title2:massaction@massaction:$user->hasRight('othermodule', 'read'):/massaction/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2
        ...
        /* Example:
        if (! isModEnabled('massaction')) $conf->massaction->enabled=0;	// This is to avoid warnings
        $this->dictionaries=array(
...
*/
		...
		//							'langs'=>'massaction@massaction',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
		//							'position'=>100,
		//							'enabled'=>'isModEnabled('massaction')',  // Define condition to show or hide menu entry. Use 'isModEnabled('massaction')' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
		//							'perms'=>'1',			                // Use 'perms'=>'$user->hasRight('massaction', 'level1', 'level2')' if you want your menu with a permission rules
		//							'target'=>'',
		//							'user'=>2);				                // 0=Menu for internal users, 1=external users, 2=both
		// $r++;
...
/*
		$this->menu[$r]=array(
			...
			'perms'=>'$user->hasRight('massaction', 'read')',			                // Use 'perms'=>'$user->hasRight('missionorder', 'level1', 'level2')' if you want your menu with a permission rules
			'target'=>'',
			'user'=>0
		);
		$r++;
		...
		$this->menu[$r]=array(
			...
			'langs'=>'massaction@massaction',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>100+$r,
			'enabled'=> 'isModEnabled('massaction')',  // Define condition to show or hide menu entry. Use 'isModEnabled('missionorder')' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=> '$user->hasRight('massaction', 'write')',			                // Use 'perms'=>'$user->hasRight('missionorder', 'level1', 'level2')' if you want your menu with a permission rules
			'target'=>'',
			'user'=>0
